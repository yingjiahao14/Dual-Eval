...
def lemma_check(answer,llm_response,nlp_pipeline,language='Korean'):
    ...
    if language == 'Korean':
        ...
        stemmer = EcsStemmer()
        answer_tokens = [stemmer.stemmWord(word.replace('(','').replace(')','')) for word in answer.split()]
        llm_tokens = [stemmer.stemmWord(word.replace('(','').replace(')','')) for word in llm_response.split()]
...
def soft_exact_match(country,language,annotation_dict,response_df,id_col,r_col,annotations_key='aggregated_answers'):
    ...
    response_df['answer_index'] = [None]*response_df.shape[0]
    ...
    for qid,data in pb:
        ...
        if llm_response and data[annotations_key]:
            ...
            for answer_index, agg_ans in enumerate(data[annotations_key]):
                if language != 'English':
        ...
        if flag:
            ...
            response_df.loc[response_df[id_col]==qid,'answer_index'] = paired_index
