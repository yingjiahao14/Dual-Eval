...
def get_model_response_file(
    ...
    if filename == None:
        if masked == None:
            ...
            template='{model}-{country}_{language}_{prompt_no}_{masked}_result.csv'
            filename =  template.replace('{model}',model).replace('{country}',country.replace(' ','_')).replace('{language}',language).replace('{prompt_no}',prompt_no).replace('{masked}', masked)
        print(filename)
    if data_dir == None:
        assert 'ERROR: No data directory given' 
