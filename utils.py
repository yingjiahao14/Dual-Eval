...
MODEL_PATHS = {
    ...
    "Llama3.1-8b-Instruct":"model/Llama-3.1-8B-Instruct",
    "Llama3-8b-70B-Instruct":"model/Llama-3-70B-Instruct",
    "Qwen2.5-7B-Instruct":"model/Qwen2.5-7B-Instruct",
    "Mistral-7B-Instruct":"model/Mistral-7B-Instruct",
    "Mistral-7B-Instruct-v0.3":"model/Mistral-7B-Instruct-v0.3",
    "command-r":"model/command-r",
    "gemma-2-9b-it":"model/gemma-2-9b-it",
    "bloomz-7b1":"model/bloomz-7b1",
}
...
def get_tokenizer_model(model_name,model_path,model_cache_dir):
    ...
    if 'gpt' not in model_name and 'gemini' not in model_name and 'claude' not in model_name and 'bison' not in model_name:
        ...
        #     print("Load model successfully")
            ...
            model = PeftModel.from_pretrained(base_model, model_path, device_map="auto",
                                                                cache_dir=os.path.join(model_cache_dir,model_path))
        elif 'Mistral-7B-Instruct-v0.3'in model_path:
            tokenizer = MistralTokenizer.from_file(f"{model_path}/tokenizer.model.v3")
            print("Load tokenizer successfully")
            model = Transformer.from_folder(model_path, device_map="auto",
                                                        torch_dtype=torch.float16,
                                                        ).eval()
            print("Load config successfully")
...
def model_inference(prompt,model_path,model,tokenizer, temperature,top_p, max_length=512):
    if 'Orion' in model_path:
        ...
        end_idx = s.find(end_token)
        result = s[len(prompt):end_idx]

    elif "gemma-2-9b-it" in model_path:
        messages = [{"role": "user", "content": prompt},]
        generator = TextGenerationPipeline(model = model, tokenizer = tokenizer)
        return_sample= [f"<bos><start_of_turn>user\n {prompt}<end_of_turn>\n<start_of_turn>model"]
        outputs = generator(
            return_sample, 
        ...
        result = outputs[0][0]['generated_text']
...
def get_model_response(model_name,prompt,model,tokenizer,temperature,top_p,gpt_azure):
    ...
    if 'gpt' in model_name:
        response = get_gpt_response(prompt,model_name=model_name,temperature=temperature,top_p=top_p)
        ...
        response = model_inference(prompt,model_path=model_name,model=model, tokenizer=tokenizer, temperature=temperature,top_p=top_p)
